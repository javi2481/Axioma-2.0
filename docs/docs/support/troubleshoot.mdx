...
When you start OpenRAG, `No container runtime found` and `Docker/Podman not ready` errors indicate that OpenRAG cannot find a running Docker or Podman machine to run the OpenRAG containers.

Before you starting OpenRAG, do the following:

1. Install Docker or Podman as well as docker-compose or podman-compose.
2. Make sure Docker or Podman is available in the PATH.
3. Make sure a container VM is running.
...
* Use a CPU-only deployment to reduce memory usage.
   ...
   For terminal-managed deployments, you must [start the terminal session with `--tui`](/tui#access-the-tui), such as `uv run openrag --tui`. Then, click **Status** in the [TUI](/tui), and then click **Switch to CPU mode**.
...
## OpenRAG fails to start from the terminal with operation not supported

This error occurs when starting OpenRAG with the terminal's **Start services** option while in the [WSL (Windows Subsystem for Linux)](https://learn.microsoft.com/en-us/windows/wsl/install).
...
To access the OpenRAG application, open a web browser, and then enter `http://localhost:3000` in the address bar.
...
Make sure your container VM has [sufficient memory](/support/troubleshoot#container-out-of-memory-errors) for processing large files and folders.

If you experience timeouts during ingestion, edit the following environment variables in your [OpenRAG `.env` file](/reference/configuration):
...
1. If an active OpenRAG terminal is running, select the **Exit** option to end the session.
...
3. Invoke OpenRAG to restart the terminal session and regenerate the cache:

   ```bash
   uvx openrag
   ```

4. Select the **Start services** option to restart the OpenRAG containers.

5. Select the **Open OpenRAG in browser** option, and then retry document ingestion.

In contrast to `uvx`, if you install OpenRAG with `uv`, dependencies are synced directly from your `pyproject.toml` file.
This should automatically install `easyocr` because `easyocr` is included as a dependency in OpenRAG's `pyproject.toml`.

If you don't need OCR, you can disable OCR-based processing in your [ingestion settings](/ingestion-configure) to avoid requiring `easyocr`.

### Problems when referencing documents in chat

If the OpenRAG **Chat** doesn't seem to use your documents correctly, [browse your knowledge base](/knowledge-browse) to confirm that the documents are uploaded in full, and the chunks are correct.
...
You can test this by applying the filter when you [browse the knowledge base](/knowledge-browse).
...
If the document chunks have missing, incorrect, or unexpected text, you must [delete the documents](/knowledge-browse#delete-knowledge) from your knowledge base, modify the [ingestion settings](/ingestion-configure) or the documents themselves, and then reingest the documents.
For example:
...
Click <Icon name="Library" aria-hidden="true"/> **Knowledge** to confirm that the documents are present in the OpenRAG OpenSearch knowledge base, and then click each document to see how the document was chunked.
If a document was chunked improperly, you might need to tweak the [ingestion settings](/ingestion-configure) or modify and reupload the document.
...
12. In a terminal, run OpenRAG, and then start the OpenRAG services with GPU mode enabled.
Or, for self-managed deployments, deploy OpenRAG with the `docker-compose-gpu.yml` file.